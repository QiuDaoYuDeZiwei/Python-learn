...
def TestProxy(proxy):
    ...
    """
    try:
        r = requests.get(url = TEST_URL, headers = get_header(), timeout = TIMEOUT, proxies = proxy)
...
def baidu_check(proxy):
    ...
    '''
    try:
        r = requests.get(url = 'https://www.baidu.com', headers = get_header(), timeout = TIMEOUT, proxies = proxy)
        r.encoding = chardet.detect(r.content)['encoding']
        if r.ok:
            print proxy
            return proxy
